...
class Ventas extends CI_Controller {
	...
	public function store(){
		...
		//id del comprobante que se haya elegido
		$idcomprobante = $this->input->post("idcomprobante");

		//id del cliente seleccionado
		$idcliente = $this->input->post("idcliente");

		//recuperamos la id de sesion para ver quien creo la venta
		$idusuario = $this->session->userdata("id");

		//recuperamos el numero de documento y sy serie
		$numero = $this->input->post("numero");
		$serie = $this->input->post("serie");

		//recuperamos los array de los prodcutos, ya que pueden ser mas de uno
		...
		if ($this->Ventas_model->save($data)) {
			...
			//debemos sumar en 1 la cantidad del comprobante, ya sea boleta o factura
...
//guarda los productos de la venta
	protected function save_detalle($productos,$idventa,$precios,$cantidades,$importes){
		//ciclo hasta la cantidad de productos ingresados
		for ($i=0; $i < count($productos); $i++) {
			...
			//FUNCION PARA RECUDICR EL STOCK DEL PRODUCTO
	...
	//SE RECUDE EL STOCK CON LA CANTIDAD COMPRADA
